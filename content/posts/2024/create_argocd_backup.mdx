...
We'll build upon the steps outlined in the [Disaster Recovery](https://argo-cd.readthedocs.io/en/latest/operator-manual/disaster_recovery/) page of their documentation.


## Backing Up Using argocd admin
We will use Docker to mount the Argo CD image of the same version of Argo CD we are backing up, mount our `.kube` folder containing our `kubectl` `config` file, and run `argocd admin export` to dump all Argo CD configs to a single YAML file.

...
<Callout>**Note**: Do not save the Terraform Cloud state file to `.terraform/terraform.tfstate` because `terraform state push` will treat it as a normal state file and will fail to upload due to Terraform Cloud using state version `4` whereas the Terraform CLI only supporting state version \<= `3`</Callout>
